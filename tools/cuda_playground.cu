...
// __global__ void reduceKernel0(const float* dArr, float* dResult, size_t size) {
//   extern __shared__ float shared[];

//   int tid = threadIdx.x;
//   int idx = blockIdx.x * blockDim.x + threadIdx.x;

//   shared[tid] = (idx < size) ? dArr[idx] : 0.0f;
//   __syncthreads();
...
//   reduceKernel0<<<gridSize, blockSize, blockSize * sizeof(float)>>>(d_input, d_intermediate, n);
