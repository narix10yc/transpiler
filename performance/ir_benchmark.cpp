...
int main(int argc, char** argv) {
    ...
    for (unsigned i = 0; i < nqubitsVec.size(); i++) {
        ...
        rst = timer.timeit(
            [&]() {
                applySingleQubitQuEST<real_t>(sv.real, sv.imag, _metaData[0].mPtr, nqubits, Q0_VALUE);
            }
        );
        // rst.display();
        tVec[i] = rst.min;
    ...
    for (unsigned i = 0; i < nqubitsVec.size(); i++) {
        ...
        rst = timer.timeit(
            [&]() {
                applySingleQubit<real_t>(sv.real, sv.imag, _metaData[0].mPtr, nqubits, q0_value);
            }
        );
        // rst.display();
        tVec[i] = rst.min;
    ...
    for (unsigned i = 0; i < nqubitsVec.size(); i++) {
        ...
        rst = timer.timeit(
            [&]() {
                applySingleQubitTemplate<real_t, Q0_VALUE>(sv.real, sv.imag, _metaData[0].mPtr, nqubits);
            }
        );
        // rst.display();
        tVec[i] = rst.min;
