...
Function* IRGenerator::generateKernelDebug(const QuantumGate& gate,
  ...
  const auto loadMatrixF = [&]() {
    ...
    for (unsigned i = 0; i < matrix.size(); i++) {
      ...
      if (shareMatrixElemThres > 0.0) {
        auto realIt =
            std::find_if(uniqueEntries.begin(), uniqueEntries.end(),
                         [thres = shareMatrixElemThres,
                          real = real](const std::pair<double, int> pair) {
        ...
        auto imagIt =
            std::find_if(uniqueEntries.begin(), uniqueEntries.end(),
                         [thres = shareMatrixElemThres,
                          imag = imag](const std::pair<double, int> pair) {
  ...
  { /* locate start pointer */
    if (_config.usePDEP) {
      ...
      if (debugLevel > 2)
        std::cerr
            << "finding masks... idxStart = sum of ((counter & mask) << i)\n";
      ...
      for (unsigned i = 0; i < higherQubits.size(); i++) {
        ...
        if (debugLevel > 2) {
          std::cerr << "i = " << i << ", bit = " << bit << ", mask = " << mask
                    << " 0b" << std::bitset<12>(mask) << "\n";
        }
      ...
      mask =
          ~((1ULL << (higherQubits.back() - sepBit - higherQubits.size() + 1)) -
            1);
      if (debugLevel > 2) {
      ...
      tmpCounterV = builder.CreateAnd(counterV, mask, "tmpCounter");
      tmpCounterV =
          builder.CreateShl(tmpCounterV, higherQubits.size(), "tmpCounter");
    ...
    for (unsigned hi = 0; hi < HK; hi++) {
      ...
      if (_config.ampFormat == IRGeneratorConfig::SepFormat) {
        pRe[hi] = builder.CreateGEP(vecType, pReArg, _idxV,
                                    "pRe." + std::to_string(hi));
        pIm[hi] = builder.CreateGEP(vecType, pImArg, _idxV,
                                    "pIm." + std::to_string(hi));
        reFull = builder.CreateLoad(vecType, pRe[hi],
                                    "re.full." + std::to_string(hi));
        imFull = builder.CreateLoad(vecType, pIm[hi],
                                    "im.full." + std::to_string(hi));
      } else {
        pSv[hi] = builder.CreateGEP(vecTypex2, pSvArg, _idxV,
                                    "pSv." + std::to_string(hi));
        svFull = builder.CreateLoad(vecTypex2, pSv[hi],
                                    "sv.full." + std::to_string(hi));
        reFull = builder.CreateShuffleVector(svFull, reSplitMask,
                                             "re.full." + std::to_string(hi));
        imFull = builder.CreateShuffleVector(svFull, imSplitMask,
                                             "im.full." + std::to_string(hi));
      }
      for (unsigned i = 0; i < LK; i++) {
        unsigned key = keyStart + i;
        real[key] = builder.CreateShuffleVector(reFull, splits[i],
                                                "real." + std::to_string(key));
        imag[key] = builder.CreateShuffleVector(imFull, splits[i],
                                                "imag." + std::to_string(key));
      }
    }
  }
  ...
  for (unsigned hi = 0; hi < HK; hi++) {
    ...
    for (unsigned li = 0; li < LK; li++) {
      ...
      if (_config.useFMS) {
        for (unsigned c = 0; c < K; c++) {
          if (matrix[r * K + c].realLoadNeg)
            newReal[li] =
                genMulSub(newReal[li], matrix[r * K + c].realVal, real[c],
                          matrix[r * K + c].realFlag, "", nameRe);
          else
            newReal[li] =
                genMulAdd(newReal[li], matrix[r * K + c].realVal, real[c],
                          matrix[r * K + c].realFlag, "", nameRe);
        }
        for (unsigned c = 0; c < K; c++) {
          if (matrix[r * K + c].imagLoadNeg)
            newReal[li] =
                genMulAdd(newReal[li], matrix[r * K + c].imagVal, imag[c],
                          matrix[r * K + c].imagFlag, "", nameRe);
          else
            newReal[li] =
                genMulSub(newReal[li], matrix[r * K + c].imagVal, imag[c],
                          matrix[r * K + c].imagFlag, "", nameRe);
        }
      } else {
        ...
        if (newRe0 != nullptr && newRe1 != nullptr)
          newReal[li] = builder.CreateFSub(newRe0, newRe1,
                                           "newRe" + std::to_string(r) + "_");
        else if (newRe0 == nullptr)
          newReal[li] =
              builder.CreateFNeg(newRe1, "newRe" + std::to_string(r) + "_");
      ...
      for (unsigned c = 0; c < K; c++) {
        if (matrix[r * K + c].realLoadNeg)
          newImag[li] =
              genMulSub(newImag[li], matrix[r * K + c].realVal, imag[c],
                        matrix[r * K + c].realFlag, "", nameIm);
        else
          newImag[li] =
              genMulAdd(newImag[li], matrix[r * K + c].realVal, imag[c],
                        matrix[r * K + c].realFlag, "", nameIm);
        if (matrix[r * K + c].imagLoadNeg)
          newImag[li] =
              genMulSub(newImag[li], matrix[r * K + c].imagVal, real[c],
                        matrix[r * K + c].imagFlag, "", nameIm);
        else
          newImag[li] =
              genMulAdd(newImag[li], matrix[r * K + c].imagVal, real[c],
                        matrix[r * K + c].imagFlag, "", nameIm);
      }
    }
    // merge
    ...
    for (unsigned round = 0; round < lk; round++) {
      if (debugLevel > 2)
        std::cerr << " round " << round << ", there are "
                  << (1 << (lk - round - 1)) << " pairs\n";
      for (unsigned pairI = 0; pairI < (1 << (lk - round - 1)); pairI++) {
