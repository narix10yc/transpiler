...
FPGAGateCategory saot::fpga::getFPGAGateCategory(
        const QuantumGate& gate, const FPGAGateCategoryTolerance& tolerances) {
    switch (gate.gateMatrix.gateKind){
        case gX:
            return FPGAGateCategory::SingleQubit | FPGAGateCategory::NonComp | FPGAGateCategory::RealOnly;
        case gY:
    ...
    if (const auto* p = gate.gateMatrix.getUnitaryPermMatrix(tolerances.upTol)) {
        if (std::all_of(p->data.begin(), p->data.end(),
                [tol = tolerances.ncTol](const std::pair<size_t, double>& d) {
                    ...
                    return (std::abs(cplx - std::complex<double>( 1.0,  0.0)) <= tol)
                        || (std::abs(cplx - std::complex<double>(-1.0,  0.0)) <= tol)
