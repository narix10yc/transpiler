...
void writeMetadataHeaderFile(
        const std::vector<GateBlock*>& allBlocks, int nqubits,
...
void writeKernelsIrFile(
    ...
    IRGenerator irGenerator(config.irConfig,
        "module_blocks" + std::to_string(blockIdx0) + "_to_" + std::to_string(blockIdx1));
...
void saot::generateCpuIrForRecompilation(
        const CircuitGraph& graph, const std::string& dir,
        const CodeGeneratorCPUConfig& config, int nthreads) {
    ...
    if (config.rmFilesInsideOutputDirectory) {
        std::error_code ec;
        for (sys::fs::directory_iterator file(dir, ec), end; file != end && !ec; file.increment(ec)) {
